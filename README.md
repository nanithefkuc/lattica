...
- The classical named lattices as first-class constructions — `Z^n`, `A_n`,
  `D_n`, `E_8` — plus nested pairs `Λ_s ⊆ Λ_c` with coset enumeration, and
  Construction A / Construction D over a caller-supplied code membership seam.
...
- Hot-path quantization and decoding: Babai rounding and nearest-plane over a
  reduced basis, the closed-form Conway–Sloane nearest-point decoders, and
  `mod Λ` (`x - Q_Λ(x)`) with dithering and Voronoi-region shaping.
...
- **Implemented:** exact fixed-width integer arithmetic; Hermite and Smith
  ...
  lattices `Z^n`/`A_n`/`D_n`/`E_8`; exact short-vector enumeration (which
  *recovers* the classical determinants, minimal norms, kissing numbers and
  theta series rather than storing them); the closed-form Conway–Sloane
  decoders; `mod Λ` with dithering; nested pairs with coset enumeration;
  Construction A/D; fraction-free Gram–Schmidt; LLL and deep-insertion LLL with
  an exact rational `δ`; Lagrange–Gauss; and Babai rounding and nearest-plane.
- **Not yet:** Schnorr–Euchner enumeration and list decoding.
...
| Feature | Result |
| --- | --- |
| default (`simd`) | reserved for `archmage`-dispatched batch kernels; currently selects the portable scalar kernels |
| `internals` | unstable implementation APIs, exempt from compatibility guarantees |

## Building

`lattica` builds on stable Rust (edition 2024, MSRV 1.89) with no extra tooling
or target-feature flags — the `simd` feature currently selects the same
portable scalar kernels as the default path, so nothing is chosen at compile
time via `#[target_feature]`:
